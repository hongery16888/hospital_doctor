...
public class PatientDHZXActivity extends BasePhotoCropActivity implements NetworkConnectListener {
    ...
    public void onRequestSucceed(Object data, String requestAction) {
        if (HttpRequest.PAT_ONLINE_TEL.equals(requestAction)){
            if (isConfirm) {
                ...
                price.setText("支付金额：" + ((PatientDHZXResponse) data).getPrice() + "元");
                pricePay = ((PatientDHZXResponse) data).getPrice();
            }
        }
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        Intent intent = new Intent(PatientDHZXActivity.this, PatientDHZXResultActivity.class);
                        intent.putExtra("did", getIntent().getStringExtra("did"));
                        intent.putExtra("orderid", orderId);
                        startActivity(intent);
                        getApp().getPatDocInfoActivity().finish();
                        finish();
                        Toast.makeText(PatientDHZXActivity.this, "支付成功",
                                Toast.LENGTH_SHORT).show();
                    } else {
                        // 判断resultStatus 为非“9000”则代表可能支付失败
                        // “8000”代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(PatientDHZXActivity.this, "支付结果确认中",
                                    Toast.LENGTH_SHORT).show();
                            ...
                            Toast.makeText(PatientDHZXActivity.this, "支付失败",
                                    Toast.LENGTH_SHORT).show();
                ...
                case SDK_CHECK_FLAG: {
                    Toast.makeText(PatientDHZXActivity.this, "检查结果为：" + msg.obj,
                            Toast.LENGTH_SHORT).show();
                    break;
                }
                default:
    ...
    public void pay() {
        ...
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                PayTask alipay = new PayTask(PatientDHZXActivity.this);
                // 调用支付接口，获取支付结果
    ...
    public void check(View v) {
        Runnable checkRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                PayTask payTask = new PayTask(PatientDHZXActivity.this);
                // 调用查询接口，获取查询结果
